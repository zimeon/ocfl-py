...
class TestAll(unittest.TestCase):
    ...
    def test__validate_manifest(self):
        """Test _validate_manifest method."""
        log = TLogger()
        iv = InventoryValidator(log=log)
        self.assertEqual(iv._validate_manifest("not a manifest"), ({}, [], set()))
        self.assertIn("E041c", log.errors)
        log.clear()
        self.assertEqual(iv._validate_manifest({"xxx": []}), ({}, [], set()))
        self.assertIn("E025b", log.errors)
        log.clear()
        self.assertEqual(iv._validate_manifest({"067eca3f5b024afa00aeac03a3c42dc0042bf43cba56104037abea8b365c0cf672f0e0c14c91b82bbce6b1464e231ac285d630a82cd4d4a7b194bea04d4b2eb7": "not an array"}), ({}, [], set()))
        self.assertIn("E092", log.errors)
        log.clear()
        iv.lax_digests = True
        self.assertEqual(iv._validate_manifest(
            {
        ...
        iv._validate_manifest({"067eca3f5b024afa00aeac03a3c42dc0042bf43cba56104037abea8b365c0cf672f0e0c14c91b82bbce6b1464e231ac285d630a82cd4d4a7b194bea04d4b2eb7": ["v1/content/a", "v1/content/a/b"]})
        self.assertEqual(log.errors, ["E101b"])

    def test__validate_fixity(self):
        """Test _validate_fixity method."""
        log = TLogger()
        iv = InventoryValidator(log=log, default_spec_version="1.0")
        iv._validate_fixity("not a fixity block", [])
        self.assertIn("E056a", log.errors)
        log.clear()
        iv._validate_fixity({"a": "b"}, [])
        self.assertIn("E056b", log.errors)
        log.clear()
        iv._validate_fixity({"md5": "f1"}, [])
        self.assertIn("E057a", log.errors)
        log.clear()
        iv._validate_fixity({"md5": {"d1": "f1"}}, [])
        self.assertIn("E057b", log.errors)
        log.clear()
        iv._validate_fixity({"md5": {"68b329da9893e34099c7d8ad5cb9c940": "f1"}}, [])
        ...
        log.clear()
        # Good case
        iv._validate_fixity({"md5": {"68b329da9893e34099c7d8ad5cb9c940": ["f1a", "f1b"],
        ...
        iv.lax_digests = True
        iv._validate_fixity({"XXX": {"digest1": ["f1a", "f1b"],
                                     "digest2": ["f2"]}},
                            ["f1a", "f1b", "f2"])
        self.assertEqual(len(log.errors), 0)

    def test__validate_version_sequence(self):
        """Test _validate_version_sequence method."""
        log = TLogger()
        iv = InventoryValidator(log=log)
        self.assertEqual(iv._validate_version_sequence("not an object"), [])
        self.assertIn("E044", log.errors)
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v2": {}}), [])
        self.assertIn("E009", log.errors)
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v02": {}}), [])
        self.assertIn("E009", log.errors)
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v1": {}, "v2": {}, "v3": {}}), ["v1", "v2", "v3"])
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v0001": {}, "v0002": {}}), ["v0001", "v0002"])
        self.assertIn("W001", log.warns)
        self.assertEqual(len(log.errors), 0)
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v1": {}, "v2": {}, "v4": {}}), ["v1", "v2"])
        self.assertIn("E010", log.errors)
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v01": {}, "v02": {}, "v03": {}, "v04": {}, "v05": {}, "v06": {}, "v07": {}, "v08": {}, "v09": {}}), ["v01", "v02", "v03", "v04", "v05", "v06", "v07", "v08", "v09"])
        self.assertEqual(len(log.errors), 0)
        log.clear()
        self.assertEqual(iv._validate_version_sequence({"v01": {}, "v02": {}, "v03": {}, "v04": {}, "v05": {}, "v06": {}, "v07": {}, "v08": {}, "v09": {}, "v10": {}}), ["v01", "v02", "v03", "v04", "v05", "v06", "v07", "v08", "v09"])
        self.assertIn("E011", log.errors)

    def test__validate_versions(self):
    ...
    def test__check_digests_present_and_used(self):
        """Test _check_digests_present_and_used."""
        log = TLogger()
        iv = InventoryValidator(log=log)
        manifest = {"file_aaa1": "aaa", "file_aaa2": "aaa", "file_bbb": "bbb"}
        iv._check_digests_present_and_used(manifest, ["aaa", "bbb"], {})
        self.assertEqual(len(log.errors), 0)
        iv._check_digests_present_and_used(manifest, ["aaa"], {})
    ...
    def test__digest_regex(self):
        ...
        self.assertEqual(iv._digest_regex(), "^[0-9a-fA-F]{32}$")
        iv.digest_algorithm = "not a digest"
        self.assertEqual(iv._digest_regex(), "^.*$")
    ...
    def test__compare_states_for_version(self):
        """Test _compare_states_for_version method."""
        log = TLogger()
        iv = InventoryValidator(log=log)
        prior = InventoryValidator(log=TLogger())
        # Same digests
        ...
        prior.inventory = {
            "versions": {"v99": {"state": {"a1d1": ["f1"], "a1d2": ["f2", "f3"]}}}}
        iv._compare_states_for_version(prior, "v99")
        self.assertEqual(log.errors, [])
        log.clear()
        # Extra in iv
        iv.inventory = {
    ...
    def test__check_content_path(self):
        """Test _check_content_path method."""
        log = TLogger()
        iv = InventoryValidator(log=log)
        cp = set()
        cd = set()
        self.assertTrue(iv._check_content_path("v1/content/anything", cp, cd))
        self.assertEqual(log.errors, [])
        self.assertEqual(cp, set(["v1/content/anything"]))
        self.assertEqual(cd, set(["v1/content"]))
        log.clear()
        iv.content_directory = "xyz"
        self.assertFalse(iv._check_content_path("v1/content/anything", cp, cd))
        self.assertEqual(log.errors, ["E042c"])
        log.clear()
        self.assertTrue(iv._check_content_path("v1/xyz/anything", cp, cd))
        self.assertEqual(log.errors, [])
        # Error cases
        log.clear()
        self.assertFalse(iv._check_content_path("1/xyz/1", cp, cd))
        self.assertEqual(log.errors, ["E042a"])
        log.clear()
        self.assertFalse(iv._check_content_path("vv1/xyz/1", cp, cd))
        self.assertEqual(log.errors, ["E042a"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/x/1", cp, cd))
        self.assertEqual(log.errors, ["E042c"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/xyz/1/", cp, cd))
        self.assertEqual(log.errors, ["E100"])
        log.clear()
        self.assertFalse(iv._check_content_path("/v1/xyz/1", cp, cd))
        self.assertEqual(log.errors, ["E100"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/xyz/", cp, cd))
        self.assertEqual(log.errors, ["E100"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/xyz//1", cp, cd))
        self.assertEqual(log.errors, ["E099"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/xyz/abc//d", cp, cd))
        self.assertEqual(log.errors, ["E099"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/xyz/abc/./d", cp, cd))
        self.assertEqual(log.errors, ["E099"])
        ...
        self.assertEqual(log.errors, ["E099"])
        log.clear()
        self.assertFalse(iv._check_content_path("v1/xyz/anything", cp, cd))
        self.assertEqual(log.errors, ["E101a"])
        # Good cases
        log.clear()
        self.assertTrue(iv._check_content_path("v1/xyz/.secret/d", cp, cd))
        self.assertEqual(log.errors, [])
        log.clear()
        self.assertTrue(iv._check_content_path("v1/xyz/.a", cp, cd))
        self.assertEqual(log.errors, [])
    ...
    def test__check_logical_path(self):
        """Test _check_logical_path method."""
        log = TLogger()
        iv = InventoryValidator(log=log)
        lp = set()
        ld = set()
        iv._check_logical_path("almost anything goes", "root", lp, ld)
        self.assertEqual(log.errors, [])
        iv._check_logical_path("including/this/long/path", "root", lp, ld)
        self.assertEqual(log.errors, [])
        iv._check_logical_path("/but not this", "root", lp, ld)
        self.assertEqual(log.errors, ["E053"])
        log.clear()
        iv._check_logical_path("./or this", "root", lp, ld)
